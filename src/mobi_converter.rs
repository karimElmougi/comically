...
pub fn create_mobi(epub_path: &Path, output_path: &Path) -> Result<()> {
    info!("Converting EPUB to MOBI: {} -> {}", epub_path.display(), output_path.display());
    ...
    if !output.status.success() && !output_str.contains("Warnings") {
        anyhow::bail!("KindleGen failed: {}", output_str);
    }
    ...
    if mobi_path != output_path {
        fs::rename(&mobi_path, output_path)
            .context(format!("Failed to move MOBI file from {} to {}", 
                mobi_path.display(), output_path.display()))?;
    }
